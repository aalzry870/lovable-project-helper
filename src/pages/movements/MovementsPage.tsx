...
const MovementsPage = () => {
  ...
  const [items, setItems] = useState<MovementItem[]>([
    { product_id: products[0]?.id || '', quantity: 0, unit: 'قطعة', notes: '' }
  ]);
  ...
  // دالة لحساب الرصيد الحالي لمنتج في مخزن محدد (تدعم الحركات المتعددة)
  ...
  const openAddMulti = () => {
    ...
    setItems([{ product_id: products[0]?.id || '', quantity: 0, unit: 'قطعة', notes: '' }]);
    setDialogOpen(true);
  };
  ...
  const addItem = () => setItems([...items, { product_id: products[0]?.id || '', quantity: 0, unit: 'قطعة', notes: '' }]);
  ...
  const handleSave = async () => {
    if (movementType === 'single') {
      ...
      if (editing) await updateMovement({ ...editing, ...form });
      else await addMovement({ ...form });
      ...
      for (let i = 0; i < items.length; i++) {
        if (!items[i].product_id) {
          toast({ title: 'خطأ', description: `الصنف ${i+1}: لم يتم اختيار منتج`, variant: 'destructive' });
          return;
        }
      ...
      // تحويل items إلى الشكل المطلوب مع تحويل الكميات إلى أرقام
    ...
    try {
      ...
      if (platform === 'android') {
        ...
        if (win) {
          ...
          link.click();
          toast({ title: 'تم تحميل الملف', description: 'تم منع فتح النافذة المنبثقة، تم تحميل الملف بدلاً من ذلك.' });
        }
      }
    } catch (error) {
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
          ...
          <div className="grid gap-3 sm:gap-4 mt-2">
            ...
            {/* جهة الصرف / المورد */}
            <div className="space-y-1.5">
              <Label className="text-xs sm:text-sm">{movementType === 'single' ? (form.entity_type === 'supplier' ? 'المورد' : 'جهة الصرف') : (multiForm.entity_type === 'supplier' ? 'المورد' : 'جهة الصرف')}</Label>
              <select
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
            ...
            {movementType === 'multi' && (
              <div className="space-y-2">
                <Label className="text-xs sm:text-sm">الأصناف</Label>
