...
import { Package, Building2, Truck, ArrowLeftRight, AlertTriangle, TrendingUp, RefreshCw, ArrowDownCircle, ArrowUpCircle, BarChart3 } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // آخر 5 حركات (يمكن عرضها في جدول)
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-card rounded-xl p-4 sm:p-5 shadow-card border border-border">
          <h3 className="text-sm sm:text-base font-semibold text-foreground mb-3 sm:mb-4 flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-destructive" />{t('dash_out_of_stock')}
          </h3>
          {outOfStock.length === 0 ? (
            <p className="text-xs sm:text-sm text-muted-foreground">{t('no_data')}</p>
          ) : (
